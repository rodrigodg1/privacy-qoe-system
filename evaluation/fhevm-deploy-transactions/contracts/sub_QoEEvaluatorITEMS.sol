...
contract sub_QoEEvaluatorITEMS is SepoliaZamaFHEVMConfig {
    struct QoEData {
        euint8 qosType;
        euint8 qodModel;
        euint8 qodOSVersion;
    ...
    function addData(
        ...
        // Update all totals
        totalMOS = TFHE.sub(totalMOS, TFHE.asEuint32(mosValue));
        totalQosType = TFHE.sub(totalQosType, TFHE.asEuint32(qosTypeValue));
        totalQodModel = TFHE.sub(totalQodModel, TFHE.asEuint32(qodModelValue));
        totalQodOSVersion = TFHE.sub(totalQodOSVersion, TFHE.asEuint32(qodOSVersionValue));
        totalQosOperator = TFHE.sub(totalQosOperator, TFHE.asEuint32(qosOperatorValue));
